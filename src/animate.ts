...
export type AnimationName =
  | 'fade' | 'slide' | 'bounce' | 'zoom' | 'flip'
  | 'lift' | 'glow' | 'pulse' | 'shake' | 'spin';
...
export const ANIMATIONS: Record<AnimationName, AnimConfig> = {} as any;
for (const name of KNOWN) {
  ANIMATIONS[name as AnimationName] = { name: name as AnimationName, duration: DEFAULT_DURATION };
}
...
export function parseAnimationToken(token: string): AnimConfig | null {
  ...
  if (!m) return null;
  const name = m[1] as AnimationName;
