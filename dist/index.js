...
export function compile(wflSource, registry) {
...
export function formatComponentOutput(output, componentName = 'Page') {
...
if (isCli) {
    ...
    try {
        ...
        if (sub === 'compile' || sub === 'build') {
            ...
            if (!target)
                throw new Error(`Missing path for ${sub} command`);
            const stat = statSync(target);
            if (stat.isDirectory()) {
                const files = readdirSync(target).filter((f) => f.endsWith('.wfl'));
                if (files.length === 0)
                    throw new Error(`No .wfl files found in ${target}`);
            ...
            const output = compile(input, reg);
            console.log('// Imports:');
