...
export function resolve(node, registry) {
    if (node.type === 'iteration')
        return resolveIteration(node, registry);
    if (node.type === 'conditional')
        return resolveConditional(node, registry);
    if (node.type !== 'component') {
        throw new Error(`Cannot resolve non-component node: ${node.type}`);
    }
    return resolveComponent(node, registry);
}
function resolveIteration(node, registry) {
...
function resolveConditional(node, registry) {
...
function resolveComponent(node, registry) {
